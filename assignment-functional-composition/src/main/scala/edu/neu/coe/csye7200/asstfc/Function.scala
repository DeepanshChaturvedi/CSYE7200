...
object Function {
   ...
   */
  // You know this one
  def lift[T, R](f: T => R): Try[T] => Try[R] = t => t map f
  // TO BE IMPLEMENTED
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] =
    // TO BE IMPLEMENTED
    {
      (try1: Try[T1], try2: Try[T2], try3: Try[T3]) =>
  ...
  {
    {
      (try1: Try[T1], try2: Try[T2], try3: Try[T3], try4: Try[T4], try5: Try[T5], try6: Try[T6], try7: Try[T7]) =>
        (try1, try2, try3, try4, try5, try6, try7) match {
    ...
    // TO BE IMPLEMENTED
  {
    for {
      ...
      t6y <- t6y
      t7y <- t7y
    } yield f(t1y, t2y, t3y, t4y, t5y, t6y, t7y)
  }
